...
\begin{bmatrix}0&0&0&1&3\end{bmatrix}\Big).$$  (See examples in \link[Subspaces of $\RR^n$ Associated with Matrices]{\xmbaseurl/VSP-0040/main} for the details.) It is easy to check that each of the basis vectors of $\mbox{null}(A)$ is orthogonal to each of the basis vectors of $\mbox{row}(A)$, demonstrating the first part of Theorem~\ref{th:4subspaces}.  You will be asked to demonstrate the second part of Theorem~\ref{th:4subspaces} for this example in Practice Problem \ref{prob:finishex4subspaces}.
\end{explanation}
\end{example}
...
$$\vec{w}=\mbox{proj}_W(\vec{x})
      =\mbox{proj}_{\vec{f}_1}(\vec{x})+\mbox{proj}_{\vec{f}_2}(\vec{x})
      = \frac{4}{2}\begin{bmatrix}
  1 \\ 0 \\ 1 \\ 0
...
\end{equation*}
\begin{enumerate}
\item\label{th:ProjLinTran_a} $T$ is a linear transformation.  (See \href{\xmbaseurl/LTR-0010/main}{Introduction to Linear Transformations}.)

\item\label{th:ProjLinTran_b} $\mbox{im}(T)$ is $W$ and $\mbox{ker}(T)$ is $ W^\perp$.  (See \href{\xmbaseurl/LTR-0050/main}{Image and Kernel of a Linear Transformation}.)
...
In this problem you are asked to finish Example \ref{ex:4subspaces}.  More specifically, for the matrix $$A=\begin{bmatrix}2&-1&1&-4&1\\1&0&3&3&0\\-2&1&-1&5&2\\4&-1&7&2&1\end{bmatrix},$$ show that $\mbox{null}(A^T) = (\mbox{col}(A))^\perp$.  It may be helpful to consult \href{\xmbaseurl/VSP-0040/main}{Subspaces of $\RR^n$ Associated with Matrices}, where we found a basis for the column space of this matrix.
\end{problem}
